...
function onLoginAttempt(message, sender, sendResponse) {
  ...
  hashPassword(message.password, salt).then(function(trimmedHash) {
    debugLog(
      'Storing attempt hash ' + trimmedHash + ' for ' + message.user,
    );
    let attempt = {
      site: message.site,
      user: message.user,
...
function onrefreshOptionsParams(message, sender, sendResponse) {
  ...
  .then((optionsJSON) => {
    ...
    let url =
      OPTIONS_URL+'?data='+encodeURIComponent(optionsJSONString);
...
function onWebRequest(request) {
  ...
  requestTimer =
    setTimeout(sendBatchedWebRequestLogs, WEB_REQUEST_BATCH_TIMEOUT);
}
