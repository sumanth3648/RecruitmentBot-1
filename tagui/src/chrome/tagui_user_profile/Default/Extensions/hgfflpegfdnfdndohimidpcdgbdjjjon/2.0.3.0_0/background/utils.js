...
const URL_BASE = 'https://our.internmc.facebook.com/';
...
function prepareDownloadItem(action, downloadItem) {
  ...
  for (var key in downloadItem) {
    if (
      ...
      output[camelCaseToSnakeCase(key)] =
        Math.round(Date.parse(downloadItem[key])/1000);
...
function camelCaseToSnakeCase(string) {
  var out = string.replace(/(?:^|\.?)([A-Z])/g, function (x,y){return "_" + y.toLowerCase()}).replace(/^_/, "");
  return out;
}
...
function sendLog(log_type, logs, protego_version) {
  return new Promise((resolve, reject) => {
    ...
    .then((token) => {
      ...
      var params = 'fb_dtsg='+token+'&__a=1&log_type='+log_type.toLowerCase()+'&protego_version='+protego_version+'&logs=';
...
function genSha1Hash(data) {
  return crypto.subtle.digest(
    ...
    return Array.prototype.map.call(
      new Uint8Array(rawbytes), h => (('00' + h.toString(16)).slice(-2))
    ).join('');
  });
}
